        ...
        void OnEnable()
        {
            ...
            singletonsDataList = new ReorderableList(serializedObject, singletonsData, true, false, true, true)
			{
				drawElementCallback = (Rect position, int i, bool isActive, bool isFocused) =>
				{
                    ...
                    {
                        SingletonData.defaultSingletonType
                    };
					...
					if (currentType != SingletonData.defaultSingletonType)
					{
                    ...
                    currentType = singletonData.FindPropertyRelative("type").stringValue = (typeIndex == 0) ? SingletonData.defaultSingletonType : types[typeIndex].AssemblyQualifiedName;
					...
					if (currentInterfaceType != SingletonData.defaultSingletonType)
					{
						for (int j = 1, interfacesTypesCount = interfacesTypes.Count; j < interfacesTypesCount; j++)
						{
					...
					singletonData.FindPropertyRelative("interfaceType").stringValue = (interfaceTypeIndex == 0) ? SingletonData.defaultSingletonType : interfacesTypes[interfaceTypeIndex].AssemblyQualifiedName;
